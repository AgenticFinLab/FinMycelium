...
def create_and_store_user_query(
    data_manager: DataManager,  logger: logging.Logger, query_text: str, key_words: List[str],
) -> UserQueryInput:
    """
...
def lm_build_pipeline_main(raw_texts: List[str], query_text: str, key_words: List[str],output_dir: str):
    ...
    user_query_input = create_and_store_user_query(
        data_manager, logger, query_text=query_text, key_words=key_words,
    )
    ...
    # lmbuilder = LMBuilder(
    #     config={"output_dir": output_dir}  # Specify output directory
    # )
