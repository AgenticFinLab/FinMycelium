...
The platform provides three types of outputs for reconstructed financial events:

1. **📦 Reconstructed Result** (`FinalEventCascade.json`)
   - Complete structured event cascade data generated by `AgentEventBuilder`
   - Contains detailed event stages, episodes, participants, and transactions
   - JSON format suitable for programmatic analysis and further processing
   - Includes confidence scores and verifiable fields for each component

2. **🗓️ Gantt Chart Visualization** (`FinalEventCascade_gantt.html`)
   - Interactive timeline visualization of the reconstructed event
   - Generated from the Reconstructed Result data
   - Displays event stages and episodes in chronological order
   - **Viewing:** Double-click the `.html` file to open in your browser for interactive exploration

3. **📊 Event Cascade Data** (`Class_Build_Event_Cascade_*.json`)
   - Structured event data generated by `ClassEventBuilder`
   - Classified event type with corresponding cascade information
   - JSON format containing event classification and reconstruction details

**Note:** The Gantt Chart Visualization is only available for `AgentEventBuilder` results, as it requires the detailed stage and episode structure that agent-based reconstruction provides.
